...
## 0. Allow the Ansible deploy user to sudo non-interactively
...
On hosts where sudo's password prompt is **generated by PAM** (you'll see a
custom prompt like `[sudo: authenticate] Password:` instead of the standard
`[sudo] password for <user>:`), sudo ignores Ansible's `-p` marker, so Ansible
never recognises the prompt and the run dies with:

```
Timeout (32s) waiting for privilege escalation prompt
```

Give the deploy user passwordless sudo so there's no prompt to detect (this also
matches how the other working hosts are configured):
...
> If you must keep password sudo, the alternative is to force sudo to honour
> Ansible's prompt by adding `Defaults passprompt_override` in a
> `/etc/sudoers.d/` file. Passwordless is simpler for an automated deploy target.
